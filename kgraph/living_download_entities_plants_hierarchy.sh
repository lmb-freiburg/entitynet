...
SELECT DISTINCT ?ent (GROUP_CONCAT(DISTINCT ?parent_; SEPARATOR=";") AS ?parents) WHERE {
  ...
  UNION {
    ?parent wdt:P171* wd:Q756 .
    ?ent wdt:P171 ?parent
  }
  MINUS {
    ?ent (wdt:P31/wdt:P279*) | wdt:P279+ wd:Q4886
  }
  MINUS {
