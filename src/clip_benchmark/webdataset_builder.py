...
def get_parser_args():
    ...
    parser.add_argument("--split", "-s", default="test", type=str, help="Dataset split to use")
    parser.add_argument(
        "--dataset-root", "-r", default="data", type=str, help="Root directory for input data"
    )
    parser.add_argument(
    ...
    parser_special.add_argument(
        "--multilabel", action="store_true", help="Flag to signal multilabel classification dataset"
    )
    ...
    * Raw binary data: use a PyTorch `Dataset` that supports `transform=PIL_to_bytes(image_format)`, and pass in `transform=None` here.
    ...
    for index, (input, output) in enumerate(tqdm(dataloader, desc="Converting")):
        ...
        if isinstance(input, str) and transform is path_to_bytes:
            ...
            extension = (
                os.path.splitext(input)[1].replace(".", "").lower().replace("jpeg", "jpg")
                or image_format
            )
        else:
