...
(This implementation loads only the parts that overlap with imagenet: ~19k images, 113 classes)
"""
...
class ObjectNetDataset(datasets.ImageFolder):
    ...
    def __init__(self, root, transform):
        ...
        self.samples = [
            d for d in self.samples if os.path.basename(os.path.dirname(d[0])) in self.label_map
        ]
