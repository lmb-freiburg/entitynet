...
class CLIPVisionCfg:
    ...
    block_type: Optional[str] = (
        None  # attention block type ('default', 'custom'), auto-selects 'custom' if any below features enabled
    )
    ...
    scale_attn_inner: bool = (
        False  # apply layer norm on attention context, before output projection
    )
...
class CLIPTextCfg:
    ...
    # Custom attention block settings
    block_type: Optional[str] = (
        None  # attention block type ('default', 'custom'), auto-selects 'custom' if any custom features enabled
    )
    qk_norm: bool = False  # apply layer norm to q and k in attention
    scaled_cosine_attn: bool = False  # use scaled cosine attention
    scale_heads: bool = False  # learnable head-specific scale applied to attention logits
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, normalize: bool = False):
        # image: (B, 3, H, W) pixels -> features: (B, D) NEW docstring
    ...
    def encode_text(self, text, normalize: bool = False):
        ...
        x = text_global_pool(
            x, text, self.text_pool_type, eos_token_id=getattr(self, "text_eos_id", None)
        )
...
def resize_text_pos_embed(state_dict, model, interpolation: str = "cut", antialias: bool = False):
    """
    NEW fixed this function to work with more kinds of models, and to use default cut interpolation
    instead of linear, because linear does not work so well for text.
    """
    old_pos_embed, actual_key = None, None
    for pre in ["", "text."]:
        if old_pos_embed is not None:
            break
        for potential_sub_key in [
            "positional_embedding",
            "transformer.embeddings.position_embeddings.weight",
            "transformer.embed_positions.weight",
        ]:
            potential_key = f"{pre}{potential_sub_key}"
            if potential_key in state_dict:
                old_pos_embed = state_dict[potential_key]
                actual_key = potential_key
                break

    if old_pos_embed is None:
        # for some models this may be fine, e.g. sinusoidal or otherwise fixed pos embeddings.
        err_msg = f"Could not find positional embedding for model {type(model)}"
        print(err_msg)
        # breakpoint()
        # raise ValueError(err_msg)
        return
    current_obj = model
    for next_attribute in actual_key.split("."):
        current_obj = getattr(current_obj, next_attribute, None)
        if current_obj is None:
            err_msg = (
                f"Found state dict text positional embedding as {actual_key} but could not find "
                f"the same parameter in the model. Error when retrieving attribute {next_attribute}"
                f". Note: if you want to use a custom text encoder from huggingface, you must use "
                f"a huggingface model (model_name starts with 'hf-hub:')"
            )
            raise ValueError(err_msg)
    model_pos_embed = current_obj
    ...
    if interpolation == "cut":
        if num_pos > old_num_pos:
            new_pos_embed = torch.empty(num_pos, width)
            new_pos_embed[:old_num_pos] = old_pos_embed
            # Initialize new positional embeddings with CLIP's standard initialization
            # (std=0.01 as used in TextTransformer.init_parameters)
            nn.init.trunc_normal_(new_pos_embed[old_num_pos:], std=0.01)
            state_dict[actual_key] = new_pos_embed
            print(f"Extend text pos emb from {old_pos_embed.shape} to {new_pos_embed.shape}")
            return
        else:
            new_pos_embed = old_pos_embed[:num_pos]
            state_dict[actual_key] = new_pos_embed
            return

    logging.warning(f"Resizing text embeddings with linear interpolation gives bad results!")
...
def get_model_preprocess_cfg(model) -> dict:  # NEW typing
