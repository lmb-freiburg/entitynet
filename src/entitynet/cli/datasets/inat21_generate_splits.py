...
infos = {
    "traindev": {
        "description": "Validation set based on iNaturalist2021 training set. "
    ...
    "trainnodev": {
        "description": "Training set based on iNaturalist2021 training set, "
...
def generate_inat21_traindev_split(inat21_dir, verbose=False):
    """
    ...
    """
    traindev_file = inat21_dir / "inat2021_traindev_split.json"
    trainnodev_file = inat21_dir / "inat2021_trainnodev_split.json"

    in_file = Path(inat21_dir) / "train.json"
    ...
    dev_ids = load_json(get_entitynet_annotations_dir() / "inat/inat2021_traindev_ids.json")
...
def main():
    dataset_dir = get_entitynet_data_dir() / "iNat/2021"
    generate_inat21_traindev_split(dataset_dir, verbose=True)
    for file in [
        ...
        if filelist_file_full.is_file():
            print(f"Already exists, skipping save: {filelist_file_full}")
            continue
