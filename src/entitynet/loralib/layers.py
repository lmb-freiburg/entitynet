    ...
    def sub_lora_data(self):
        ...
        for param_name, lora_name in self.params_with_lora.items():
            if self.training and self.dropout_rate > 0:
                eval(f"self.{param_name}").data -= (
                    self.merge_BA_with_mask(param_name) * self.scaling
                )
            else:
                eval(f"self.{param_name}").data -= self.merge_BA(param_name) * self.scaling
...
class Embedding(nn.Embedding, LoRALayer):
    ...
    def __init__(
        self, num_embeddings: int, embedding_dim: int, r: int = 0, lora_alpha: int = 1, **kwargs
    ):
...
class LinearLoRA(nn.Linear, LoRALayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            in_features=existing_linear.in_features, out_features=existing_linear.out_features
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            existing_bias = (
                existing_mha.in_proj_bias.data if existing_mha.in_proj_bias is not None else None
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0 and any(enable_lora):
            self.w_lora_A = nn.Parameter(self.weight.new_zeros((r * sum(enable_lora), in_features)))
            self.w_lora_B = nn.Parameter(
