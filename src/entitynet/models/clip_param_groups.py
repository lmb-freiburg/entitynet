...
def get_clip_param_groups(model: nn.Module, config: Config):
    ...
    learning_rate: float = opt_cfg.hparams["lr"]
    use_lora = False
    ...
    vision_decay_factor = opt_cfg.vision_layer_decay_factor
    text_decay_factor = opt_cfg.text_layer_decay_factor
    if vision_decay_factor == 1 and text_decay_factor == 1:
        ...
        for param_name, param in named_parameters:
            ...
            if group_name not in param_groups_dict:
                ...
                assert param_groups_dict[group_name]["lr"] == learning_rate
    ...
    if vision_decay_factor < 1 or text_decay_factor < 1:
        ...
        for param_name, param in named_parameters:
            ...
            if opt_cfg.layer_decay_output_layer_has_highest_lr:
                ...
                learning_rate_here = learning_rate * decay_factor ** (
                    num_max_layer - layer_number + 1
                )
            else:
    ...
    raise ValueError(f"violated 0 <= {vision_decay_factor=} <= 1 or 0 <= {text_decay_factor=} <= 1")
...
def check_requires_weight_decay(param_name, param):
    ...
    if param_name == "logit_scale":
        return False
    if param_last_name in set(("bias", "in_proj_bias", "logit_bias", "q_bias", "v_bias")):
        return False
